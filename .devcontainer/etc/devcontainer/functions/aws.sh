...
# Function: lectusAWSPopulateConfig
# Description:
...
#
# Usage: lectusAWSPopulateConfig
lectusAWSPopulateConfig() {
  ...
  #   --trim-role-name: Remove 'ideas' prefix from role names
  #   --trim-account-name: Remove 'ideas-avm-' prefix from account names
  #   --existing-config-action overwrite: Replace existing config
  aws-sso-util configure populate \
    --account-name-case lower \
    --role-name-case lower \
    # --trim-role-name '^ideas' \
    # --trim-account-name '^ideas-avm-' \
...
# Usage: lectusAWSListProfiles
lectusAWSListProfiles() {
  echo "🟢 Listing available AWS SSO profiles"

  # List profiles using aws-sso-util
  aws-sso-util roles || {
    echo "🔴 Failed to list AWS SSO profiles"
    return 1
  }
...
lectusAWSProfile() {
  ...
  profile=$1
  if [ -z "$profile" ]; then
    echo "🟡 Usage: lectusAWSProfile <profile>"
    return 1
  fi
  ...
  lectusAWSExport "$profile" || return 1
...
lectusAWSLogin() {
  ...
  profile=$1
  if [ -z "$profile" ]; then
    echo "🟡 Usage: lectusAWSLogin <profile>"
    return 1
  fi
...
lectusAWSExport() {
  ...
  if [[ -z "$profile" ]]; then
    echo "🔴 No AWS_PROFILE set. Run 'lectusAWSLogin' or 'lectusAWSProfile' first."
    return 1
  fi
  ...
  if ! creds=$(aws configure export-credentials --profile "$profile" --format env 2>/dev/null); then
    echo "🔴 Failed to export credentials; did you run 'lectusAWSLogin $profile'?"
    return 1
  fi
  eval "$creds"
