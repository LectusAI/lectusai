...
#   $1 - Environment name (dev|stage|prod)
#
# Usage: lectusTerraformPlan prod
lectusTerraformPlan() {
...
_ideas_terraform_setup() {
  ...
  if [ -z "$AWS_SESSION_TOKEN" ]; then
    echo "🔴 AWS credentials missing, run ideasAWSLogin"
    return 1
  fi

  # Set GitHub token
  ideasGitHubExport || return 1
